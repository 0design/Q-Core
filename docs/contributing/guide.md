# Contribute to qloops

You can contribute a documentation fix, a bug fix, a Registry loop or component,
...
capability or a substantial change, first [open an issue](https://github.com/0design/qloops/issues)
...
1. Open [0design/qloops](https://github.com/0design/qloops) and click **Fork**.
2. Clone your fork, replacing YOUR-USERNAME below.
3. Create a branch from the current upstream main:

```sh
git clone https://github.com/YOUR-USERNAME/qloops.git
cd qloops
git remote add upstream https://github.com/0design/qloops.git
...
| Registry loop | `registry/loops/<id>.yaml` and `registry/catalog.source.json` | Purpose, version, author, origin, license, exact compatible engine and dependency pins |
...
| Demo evidence | `registry/demos/<id>.txt` and catalog source metadata | A reviewable result tied to an existing loop/template and exact versions |
...
The root `loops/` and `catalog.json` are legacy bundled examples. Adding a file
there alone does not publish a Registry entry. `registry/catalog.json` is
generated; edit source metadata and regenerate it instead of editing it by hand.
...
For a loop, also validate your actual file (replace YOUR-LOOP):
...
npm pack --pack-destination "$candidate_dir" --json > "$candidate_dir/pack.json"
...
`0design/qloops:main` and the head is your fork's contribution branch. Follow the
...
Registry-related changes normally run `validate / check` and `Registry / registry`.
Workflows use path filters, so a documentation-only PR may not trigger them.
Report your local checks even when GitHub has no applicable workflow run.
...
Comment on your PR, or [open an issue](https://github.com/0design/qloops/issues)
