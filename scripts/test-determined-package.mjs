...
const root=resolve('.'),sandbox=mkdtempSync(join(tmpdir(),'qloops-determined-install-'));
...
try {
  ...
  copyFileSync(join(sandbox,'node_modules/qloops/examples/determined-caller.mjs'),join(sandbox,'caller.mjs'));
