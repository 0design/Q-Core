...
if (decisions.some(review => review.state === 'CHANGES_REQUESTED') ||
    !decisions.some(review => review.state === 'APPROVED' && review.commit_id === pr.head.sha && review.user.id !== pr.user.id))
  throw Error('Current PR head requires an approval and no outstanding requested changes');
console.log(JSON.stringify({ approvedMergedPR: pr.number, commit }));
