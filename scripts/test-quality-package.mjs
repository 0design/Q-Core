...
const [aindfRoot,dsPath,unslopRoot]=process.argv.slice(2);assert.ok(unslopRoot,'Pass aindf root, synthetic DS path, unslop root');
const root=resolve('.'),sandbox=mkdtempSync(join(tmpdir(),'qloops-quality-install-'));
...
try {
  ...
  writeFileSync(join(sandbox,'config.json'),JSON.stringify({aindfRoot:resolve(aindfRoot),dsPath:resolve(dsPath),unslopRoot:resolve(unslopRoot),
    fixtureRoot:join(sandbox,'node_modules/qloops/examples/quality'),browserRoot:join(root,'docs/delivery/browser'),emptyDs:join(sandbox,'empty-ds')}));
  ...
  writeFileSync(join(root,'docs/delivery/quality-package.json'),JSON.stringify(evidence,null,2)+'\n');
  console.log(JSON.stringify({package:packed.version,sha256:evidence.sha256,readiness:[result.aindf.readiness.status,result.aindf.badReadiness.status],
    composition:result.aindf.composition.map(r=>r.native.findings[0].outcome),unslop:result.unslop.results.map(r=>r.result.status)}));
