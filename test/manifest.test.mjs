/**
 * `qf validate` is only worth running if it catches the things that would
...
test("a version this build does not read is named, not guessed at", () => {
  assert.throws(
    () => validateManifest(parseYaml(`manifest: qf.loop/v2\nid: t\nsteps: [{id: a, kind: fetch}]`)),
...
test("agent-call is reserved in the format and refused in this build", () => {
  assert.throws(
    () => validateManifest(parseYaml(`manifest: qf.loop/v1\nid: t\nsteps:\n  - id: a\n    kind: agent-call`)),
...
test("required per-kind fields are caught at validate time, not at run time", () => {
  assert.throws(() => validateManifest(parseYaml(`manifest: qf.loop/v1\nid: t\nsteps:\n  - id: a\n    kind: fetch`)), /config.url|"url"/);
