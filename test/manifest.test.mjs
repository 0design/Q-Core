/**
 * `q-core validate` is only worth running if it catches the things that would
...
const base = (extra = "") =>
  parseYaml(`manifest: q-core.workflow/v1
id: t
steps:
  - id: a
    kind: api-request
...
test("agent-call is reserved in the format and refused in this build", () => {
  assert.throws(
    () => validateManifest(parseYaml(`manifest: q-core.workflow/v1\nid: t\nsteps:\n  - id: a\n    kind: agent-call`)),
...
test("required per-kind fields are caught at validate time, not at run time", () => {
  assert.throws(() => validateManifest(parseYaml(`manifest: q-core.workflow/v1\nid: t\nsteps:\n  - id: a\n    kind: fetch`)), /config.url|"url"/);
