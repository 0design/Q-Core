...
test("resolveCatalogRoots uses QFACTORY_REGISTRY overlay", () => {
  ...
  assert.equal(r.workflowsDir, join("/tmp/reg", "workflows"));
  assert.equal(r.registryDir, "/tmp/reg");
});

test("buildCatalog returns empty workflows when dir is missing", () => {
  const cat = buildCatalog("/no/such/workflows-dir-q-core", { registryDir: "/no/such/registry" });
  assert.equal(cat.workflows.length, 0);
